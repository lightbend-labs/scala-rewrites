...
{
  def this() = this(LazyValue.later(() => ScalafixGlobal.newCompiler(Nil, Nil, Map.empty).toOption.get))
  ...
  override def withConfiguration(config: Configuration) = {
    ...
    if ((compileSv.take(4) != runtimeSv.take(4)) && config.scalacOptions.nonEmpty) {
      ...
      Configured.ok(new ExplicitNonNullaryApply(LazyValue.later { () =>
        ScalafixGlobal.newCompiler(config.scalacClasspath, config.scalacOptions, Map.empty).toOption.get
      }))
    }
  }
